...
const freezeResponse = <T extends Record<string, unknown>>(payload: T): Readonly<T> =>
...
const extractTextFromResponse = (payload: unknown): string => {
  ...
  if (Array.isArray(payload)) return payload.map(extractTextFromResponse).join('');
  if (typeof payload === 'object') return Object.values(payload).map(extractTextFromResponse).join('');
  return '';
};
 ...
 */
export async function detectJudol(
  inputText: string,
  apiKey: string,
  model = 'gpt-5-mini'
  ...
  const payload = (await response.json()) as { output?: unknown; choices?: unknown };
